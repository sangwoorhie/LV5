...
likeCount = async(commentId) => {
    const likeCount = await CommentLikes.count({where: {commentId: Number(commentId)}});
    return likeCount;
}
...
clickedUser = async(commentId, userId) => {
    const clickedUser = await CommentLikes.findOne({where: {[Op.and]: [{commentId: Number(commentId)}, {UserId: Number(userId)}]}});
    return clickedUser;
};
