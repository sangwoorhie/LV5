...
class CommentReportRepository {
  ...
  createReport = async (postId, commentId, userId, content) => {
    const createCommentReport = await CommentReport.create({
      postId,
      commentId,
      reportUserId: userId,
      content,
  ...
  clickedUser = async (commentId, userId) => {
    const clickedUser = await CommentReport.findOne({
      where: {
        [Op.and]: [
          { commentId: Number(commentId) },
          { reportUserId: Number(userId) },
        ],
      },
    });
    return clickedUser;
  ...
  deleteReport = async (postId, commentId, userId) => {
    const deleteCommentReport = await CommentReport.destroy({
      where: { postId: postId, reportUserId: userId, commentId },
    });
