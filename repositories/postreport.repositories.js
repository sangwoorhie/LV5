...
class PostReportRepository {
  ...
  createReport = async (postId, content, userId) => {
    const createReport = await PostReport.create({
      PostId: postId,
      content,
      reportUserId: userId,
    });
  ...
  clickedUser = async (postId, userId) => {
    const clickedUser = await PostReport.findOne({
      where: {
        [Op.and]: [
  ...
  deleteReport = async (postId, userId) => {
    const deleteReport = await PostReport.destroy({
      where: { PostId: postId, reportUserId: userId },
    });
    return deleteReport;
  };
}
