...
class CommentReportService {
  ...
  createReport = async (postId, commentId, userId, content) => {
    if (!postId) {
      ...
      return { status: 404, message: "댓글이 조회되지 않습니다." };
    } else if (!content) {
      return { status: 412, message: "정당한 신고 사유를 작성해주세요." };
    } else if (!userId) {
      return { status: 403, message: "로그인 후 사용할 수 있는 기능입니다." };
    }
    ...
    if (!findComment) {
      return { status: 404, message: "댓글이 존재하지 않습니다." };
    }

    const clickedUser = await this.commentReportRepository.clickedUser(
      commentId,
      userId
    );
    if (clickedUser) {
      return { status: 403, message: "이미 신고한 댓글입니다." };
    }

    const createReport = await this.commentReportRepository.createReport(
      postId,
      commentId,
      userId,
      content
