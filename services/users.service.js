...
class UserService {
  ...
  signupUser = async (
    ...
    try {
      if (!email) {
        ...
        return {
          status: 412,
          message: "비밀번호에 닉네임을 포함해서는 안 됩니다.",
          userInfo: null,
        };
        ...
        return { status: 412, message: "나이를 입력해주세요.", userInfo: null };
      } else if (!gender) {
        return { status: 412, message: "성별을 입력해주세요.", userInfo: null };
      ...
      const createUserData = await this.userRepository.signupUser(
        email,
        password,
        nickname,
        age,
  ...
  loginUser = async (email, password) => {
    try {
      if (!email) {
        return { status: 412, message: "이메일을 입력해주세요.", Token: null };
      } else if (!password) {
        return {
          status: 412,
          message: "비밀번호를 입력해주세요.",
      ...
      } else if (loginData && loginData.password !== password) {
        return {
          status: 409,
  ...
  updateUser = async (
    ...
    try {
      ...
      } else if (newPassword.includes(nickname)) {
        return {
          status: 412,
          message: "비밀번호에 닉네임을 포함해서는 안 됩니다.",
          userInfo: null,
  ...
  deleteUser = async (userId, email, password) => {
    try {
      if (!email) {
        return { status: 412, message: "이메일을 입력해주세요" };
      } else if (!password) {
        return { status: 412, message: "비밀번호를 입력해주세요" };
      }

      const findUser = await this.userRepository.findUserById(userId);
      if (!findUser) {
        return { status: 404, message: "회원정보가 존재하지 않습니다." };
      } else if (email !== findUser.email) {
        return { status: 403, message: "이메일이 일치하지 않습니다." };
