...
class PostReportService {
    ...
    createReport = async(postId, content, userId) => {
        if (!postId) {return {status:404, message: "게시글이 존재하지 않습니다."}}
        else if (!content){return {status:412, message: "정당한 신고 사유를 기재해주세요."}}
        else if (!userId) {return {status:404, message: "로그인 후 사용할 수 있는 기능입니다."}}

        const clickedUser = await this.postReportRepository.clickedUser(postId, userId)
        if (clickedUser) {return {status:403, message: "이미 신고한 게시글입니다."}}
    ...
    deleteReport = async(postId, userId) => {
        ...
        const clickedUser = await this.postReportRepository.clickedUser(postId, userId)
        if (!clickedUser) {return {status:403, message: "본인이 신고한 게시글만 취소요청이 가능합니다."}}
