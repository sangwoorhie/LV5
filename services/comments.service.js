...
class CommentService {
  ...
  findCommentsById = async (postId) => {
    try {
      ...
      const findComments = await this.commentRepository.findCommentsById(
        findPost.postId
      ); // s붙음 댓글의 배열
      if (!findComments) {
        return {
  ...
  createComment = async (postId, userId, comment) => {
    try {
      if (!postId) {
        ...
        return { status: 403, message: "로그인 후 이용해주세요." };
      } else if (!comment) {
        return { status: 412, message: "댓글을 입력해주세요." };
      }
  ...
  updateComment = async (userId, postId, commentId, comment) => {
    try {
      if (!postId) {
        ...
        return { status: 412, message: "수정 댓글을 입력해주세요." };
      }
      ...
      if (!findComment) {
        return { status: 403, message: "댓글이 존재하지 않습니다." };
      }
